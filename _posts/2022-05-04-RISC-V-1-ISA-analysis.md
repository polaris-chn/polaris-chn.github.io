...
## RV64I
首先需要明确一点，RV64I和RV32I的指令宽度都是32bit。RV64I共有64条指令，包括59条基本指令，5条特权指令。59条基本指令中包括47条从RV32I指令发展而来的基本指令，这些指令大部分RV64I和RV32I通用，有小部分有细微差别；还包括12条RV64I独有的指令。总的指令分类有R型、I型、S型、U型，在S型和U型下面还有两个分支为SB型和UJ型。具体的指令分析如下所示。
...
### UJ型指令
属于RV31I和RV64I的UJ型指令只有一条，其格式如下所示:
...
|lw |imm[11:0]|rs1|010|rd|0000011|x[rd]=sext(M[x[rs1]+sext(imm)][31:0])，从内存中读取一个字，地址是rs1和imm符号扩展后的加和，节符号扩展后写入rd|
...
|slliw  |{0000000,shamt}|rs1|001|rd|0011011|x[rd]=sext((x[rs1] << shamt)[31:0])，rs1截断低32位进行逻辑左移，左移位数由5bit的shamt决定，移位后符号位扩展写入rd|
|srliw  |{0000000,shamt}|rs1|101|rd|0011011|x[rd]=sext((x[rs1] >> u shamt)[31:0])，rs1截断低32位进行逻辑右移，右移位数由5bit的shamt决定，移位后符号位扩展写入rd|
|sraiw  |{0100000,shamt}|rs1|101|rd|0011011|x[rd]=sext((x[rs1] >> s shamt)[31:0])，rs1截断低32位进行算术右移，右移位数由5bit的shamt决定，移位后符号位扩展写入rd|
