...
**<font color=red>注意 sext即符号位扩展；shamt即shift amount即移位数量；u代表视为无符号数，s代表视为有符号数；tmp都是32位的</font>**
...
|指令|func7|rs2|rs1|func3|rd|opcode|操作|
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
|addw   |0000000|rs2|rs1|000|rd|0111011|tmp[31:0]=rs1[31:0]+rs2[31:0], rd=sext(tmp[31:0]);先截取rs1和rs2低32位，将其和符号扩展写入xd|
|subw   |0100000|rs2|rs1|000|rd|0111011|tmp[31:0]=rs1[31:0]-rs2[31:0], rd=sext(tmp[31:0]);先截断rs1和rs2低32位，将其差符号扩展写入xd|
|sllw   |0000000|rs2|rs1|001|rd|0111011|tmp[31:0]=rs1[31:0] << rs2[4:0], rd=sext(tmp[31:0]);rs1低32bit逻辑左移，低位填0，rs2低5位为移动位数|
|srlw   |0000000|rs2|rs1|101|rd|0111011|tmp[31:0]=rs1[31:0] >> u rs2[4:0], rd=sext(tmp[31:0]);rs1低32bit逻辑右移，高位填0，rs2低5位为移动位数|
|sraw   |0100000|rs2|rs1|101|rd|0111011|tmp[31:0]=rs1[31:0] << s rs2[4:0], rd=sext(tmp[31:0]);rs1低32bit算术右移，高位填rs1[31]，rs2低5位为移动位数|
...
|auipc  |imm[19:0]|rd|0010111|x[rd]=pc+sext(imm[31:12] << 12)，20bit立即数逻辑左移12位，低位填0，符号扩展到64位，然后加上pc写入rd|
...
|指令|imm[11:0]|rs1|func3|rd|opcode|操作|
|---|:---:|:---:|:---:|:---:|:---:|:---:|
|addiw  |imm[11:0]|rs1|000|rd|0011011|tmp[31:0]=rs1[31:0]+sext(imm)[31:0], rd=sext(tmp); rs1低32位和imm符号扩展后的低32位相加，将和进行符号扩展写入xd，忽略算术溢出|
...
|remw|低32位有符号取余|0000001|rs2|rs1|110|rd|0111011|rs1[31:0]%rs2[31:0])[31:0], rd=sext(tmp[31:0]); 除数为0，求余结果为被除数[31]位符号位扩展后的结果，产生overflow时，余数结果为0x0|
